...
$CMAKE_ARGS = "  -DCUDA_TOOLKIT_ROOT_DIR=`"C:/Program Files/NVIDIA GPU Computing Toolkit/CUDA/v12.8`" -DCUDA_CUDART_LIBRARY=`"C:\Program Files\NVIDIA GPU Computing Toolkit\CUDA\v12.8\lib\x64\cudart.lib`"   -DMNN_SEP_BUILD=OFF -DMNN_BUILD_TRAIN=OFF -DMNN_BUILD_OPENCV=ON -DMNN_IMGCODECS=ON  -DMNN_OPENCL=ON -DMNN_VULKAN=ON  -DMNN_CUDA=ON -DMNN_AVX512=ON -DMNN_LOW_MEMORY=ON"
# $CMAKE_ARGS = " -Wno-dev  -DMNN_SEP_BUILD=OFF -DMNN_BUILD_TRAIN=OFF -DMNN_BUILD_OPENCV=ON -DMNN_IMGCODECS=ON  -DMNN_OPENCL=ON -DMNN_VULKAN=ON  -DMNN_CUDA=ON -DMNN_AVX512=ON -DMNN_LOW_MEMORY=ON"
